...
public class TcpClientWrapperTests
{
    ...
    [Test]
    public async Task SendMessageAsync_ShouldThrow_WhenStreamNotWritable()
    {
        var client = new TcpClientWrapper(Host, Port);
        var stream = new ReadOnlyNetworkStream();
        ...
        try
        {
            ...
            typeof(TcpClientWrapper).GetField("_stream", BindingFlags.NonPublic | BindingFlags.Instance)?.SetValue(client, stream);
    ...
    [Test]
    public void Disconnect_ShouldHandle_ExceptionsDuringCleanup()
    {
        var client = new TcpClientWrapper(Host, Port);
        var stream = new ThrowingNetworkStream();
        ...
        try
        {
            typeof(TcpClientWrapper).GetField("_tcpClient", BindingFlags.NonPublic | BindingFlags.Instance)?.SetValue(client, connectedClient);
            typeof(TcpClientWrapper).GetField("_stream", BindingFlags.NonPublic | BindingFlags.Instance)?.SetValue(client, stream);
            typeof(TcpClientWrapper).GetField("_cts", BindingFlags.NonPublic | BindingFlags.Instance)?.SetValue(client, new CancellationTokenSource());
